...
<div class="content">
        <nav class="mb-2" aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="#!">Gestion de élèves</a></li>
                ...
                <table class="table fs-9 mb-0">
                  <thead>
                    <tr>
                      ...
                      <th class="sort align-middle text-start ps-4" scope="col" data-sort="price">PRENOM</th>
                      <th class="sort align-middle text-start ps-4" scope="col" data-sort="price">CLASSE</th>
                      <th class="sort align-middle ps-4" scope="col" data-sort="category">DATE DE NAISSANCE</th>
                      <th class="sort align-middle ps-3" scope="col" data-sort="tags">LIEU DE NAISSANCE</th>
                      <th class="sort align-middle fs-8 text-center ps-4" scope="col">PARENTS</th>
                      {{-- <th class="sort align-middle ps-4" scope="col" data-sort="vendor" style="width:200px;">MERE</th> --}}
                      <th class="sort align-middle ps-4" scope="col" data-sort="time">DATE</th>
                  ...
                  <tbody class="list" id="products-table-body">
                    ...
                    <tr class="position-static">
                        ...
                        <td class="align-middle white-space-nowrap py-0">
                            <a class="d-block border border-translucent rounded-2" target="_blank" href="{{$eleve ->photo}}">
                                <img src="/storage/{{$eleve ->photo}}" alt="" width="53"/>
                            </a>
                        </td>
                        <td class="product align-middle ps-4"><a class="fw-semibold line-clamp-3 mb-0" href="#">{{$eleve->first_name}}</a></td>
                        <td class="price align-middle white-space-nowrap text-start fw-bold text-body-tertiary ps-4">{{$eleve -> last_name}}</td>
                        <td class="price align-middle white-space-nowrap text-start fw-bold text-body-tertiary ps-4">{{$eleve -> classe}}</td>
                        <td class="category align-middle white-space-nowrap text-body-quaternary fs-9 ps-4 fw-semibold">{{ \Carbon\Carbon::parse($eleve->birthday)->format('M j, Y') }}</td>
                        <td class="tags align-middle review pb-2 ps-3"><a class="text-decoration-none" href="#!"><span class="badge badge-tag me-2 mb-2">{{$eleve -> city}}</span></a></td>
                        <td class="tags align-middle review pb-2 ps-3">
                            @foreach ($eleve->his_parents as $his_parent)
                                <span class="badge badge-tag me-2 mb-2">{{ $his_parent->first_name }} {{ $his_parent->last_name }}</span>
                            @endforeach
                        </td>
                        {{-- <td class="price align-middle white-space-nowrap text-end fw-bold text-body-tertiary ps-4">{{$eleve -> mother_id}}</td> --}}

                        {{-- @if ($eleve->status == 1)
                        <td class="align-middle white-space-nowrap text-end statuses"><span class="badge badge-phoenix fs-10 badge-phoenix-success">Publié</span></td>
                        @else
                            <td class="align-middle white-space-nowrap text-end statuses"><span class="badge badge-phoenix fs-10 badge-phoenix-danger">Non publié</span></td>
                        @endif --}}
                        {{-- <td class="vendor align-middle text-start fw-semibold ps-4"><a href="#!">{{$eleve->add_by}}</a></td> --}}
                        <td class="time align-middle white-space-nowrap text-body-tertiary text-opacity-85 ps-4">{{ \Carbon\Carbon::parse($eleve->created_at)->format('M j, Y') }}. {{ \Carbon\Carbon::parse($eleve->created_at)->format('H:i') }}</td>
                        <td class="align-middle white-space-nowrap text-start pe-0 ps-4 btn-reveal-trigger">
                          <div class="btn-reveal-trigger position-static">
                            ...
                            <div class="dropdown-menu dropdown-menu-end py-2">
                              ...
                              <a class="dropdown-item" href="#InboxParent" data-bs-toggle="modal" data-product-id="{{$eleve->id}}">Contacter les parents</a>
                              ...
                              {{-- @if ($eleve->en_promotion == 0)
                                <a class="dropdown-item" href="#projectsBoardViewModalB" data-bs-toggle="modal" data-product-id="{{$eleve->id}}">Faire une promotion</a>
                              @else
                                <a class="dropdown-item" href="#projectsBoardViewModalB" data-bs-toggle="modal" data-product-id="{{$eleve->id}}">Editer la promotion</a>
                                <a class="dropdown-item" href="{{route('admin_reset_promotion', ['product_id' => $eleve->id])}}">Annuler la promotion</a>
                              @endif
                              @if ($eleve->status ==0)
                                <a class="dropdown-item" href="#" onclick="confirmPublish('{{ route('admin_product_publish', ['code' => $eleve->code]) }}')">Publier sur la plateforme</a>
                              @endif
                              <div class="dropdown-divider"></div>
                                @if ($eleve->status==1)
                                    <a class="dropdown-item text-danger" href="#!" onclick="confirmNotPublish('{{ route('admin_product_no_publish', ['code' => $eleve->code, 'id_user' => Auth::guard('admin')->user()->id]) }}')">Annuler la publication</a>
                                @endif --}}
                                {{-- onclick="confirmDelete('{{ route('admin_product_delete', ['code' => $eleve->code])}}')" --}}
                              <a class="dropdown-item text-danger" href="#!" onclick="confirmDelete('{{ route('admin_students_delete', ['token' => $eleve->token])}}')">Supprimer</a>
                            </div>
                          </div>
                        </td>
                      </tr>
